...
#######################################################
write.csv(cbind(data[,c(1:2,(7+M):(8+M+N))]),"data.csv",row.names=FALSE)
...
for (i in 1:length(name2)){
  ...
  #print(state)
  state[[i]] <- X[which(X$NAME == name2[i]),]
  #print("First")
  #print(state[[i]][1,4])
  area[[i]] <- state[[i]][1,4]
  #print("second")
...
table <- data.frame(name2,statearea)
...
#######################################################
write.table(colnames(data[(7+M):(8+M+N)]),"names.csv",row.names=FALSE,col.names=FALSE)

#######################################################
## Create set "index" for use in GAMS script  
#######################################################
number <- seq(1,length(data[,(7+M):(8+M+N)])*M,by = 1)
